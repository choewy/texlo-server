...
@Processor('voice-sync.supertone')
export class SupertoneVoiceSyncProcessor extends WorkerHost {
  ...
  async process(job: VoiceSyncJob): Promise<void> {
    ...
    const raws = await this.supertoneClient.getVoices();
    const targets = raws.map((voice) => Voice.fromSupertone(voice));
