...
export class VimeoVideoDownloadProcessor extends WorkerHost {
  ...
  async process(job: VideoDownloadJob): Promise<VideoDownloadJobReturnValue | null> {
    ...
    const ytDlp = new YtDlp({ url: videoDownload.origin, debug: true });
    const [thumbnailImageUrl, { stream, title }] = await Promise.all([ytDlp.thumbnail().url(), ytDlp.mergeFormat('mp4').video().stream()]);

    const thumbnail = thumbnailImageUrl ? (await this.storageClient.uploadUrl(thumbnailImageUrl)).url : null;
    const { url, size } = await this.storageClient.uploadStream(stream);

    return { title, url, size, thumbnail };
  }
